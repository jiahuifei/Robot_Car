...
float PosionPID_Realize(PID *pid, float Actual_value) {
  ...
  /*PID算法实现*/
  pid->Output_value = pid->Kp * pid->NowError + pid->Ki * pid->Integral + pid->Kd * (pid->NowError - pid->LastError);
  /*误差传递*/
...
float AddPID_Realize(PID *pid, float Actual_value) {
  ...
  pid->NowError = pid->Target_value - Actual_value;
  /*PID算法实现，照搬公式*/
  pid->Output_value = pid->Kp * (pid->NowError - pid->LastError) + pid->Ki * pid->NowError + pid->Kd * (pid->NowError - 2 * pid->LastError + pid->PrevError);
  /*误差传递*/
